#!/usr/bin/python3
"""
Plot SAR from file(s) in CSV format.
"""
import argparse
...
parser.add_argument('files', help="CSV input files", nargs="+")
...
parser.add_argument('-y', '--ylabel', help="Alternative y-label. Default: SAR [kg/W].")
parser.add_argument('-l', '--lineskip', help="Number of rows to skip in the input file(s). Default: 1.")
...
if (args.lineskip):
    ...
    m = loadtxt(args.files[0], skiprows=1, delimiter=args.delimiter, usecols=(0,1))
    f = m[:,0].T
    y = m[:,1].T
...
for thisfile in args.files[1:]:
    if (args.lineskip):
        ...
        m = loadtxt(thisfile, skiprows=1, delimiter=args.delimiter, usecols=(0,1))
        f = vstack((f, m[:,0].T))
        y = vstack((y, m[:,1].T))
    idx = idx + 1
...
if 1:
    ...
    #     text(x, 0.7, x, ha='center', va='bottom', bbox=dict(fc='white', ec='none', pad=0))
    # ylim(0, 1)
    # yticks([-24, -18, -12, -6, 0])
    if (args.xlabel):
        xlabel(args.xlabel)
