\section[Intro]{Introduction (Henrik)}
%Motivation
\begin{frame}
    \frametitle{Introduction -- Motivation}
    \begin{block}{Tunable Antennas}
      \begin{itemize}
      \item Smartphones are generally growing in physical size, less space is allocated for the antennas due to screen, etc..
      \item LTE standard requires terminals to operate in a wide range of frequencies (700-2700 \si{MHz}) and now 450\si{MHz}
...
%Problem anal
\section[Problem Analysis]{Problem Analysis (Henrik)}
\begin{frame}[fragile]
...
\section[Preliminary Designs]{Preliminary Designs (Henrik)}
\begin{frame}
    ...
    \begin{block}{Three different antenna designs}
      ...
      \item Simplified simulations (No losses, no traces and connectors, no transmission line)
      \end{itemize}
    \end{block}
\vspace*{-0.5cm}
  \begin{center} 
...
\begin{frame}
    \frametitle{Free-space simulations -- Dual-feed (S-parameters)}
...
\begin{frame}
    \frametitle{Free-space simulations -- Dual-feed (Efficiency and correlation)}
    \vspace*{-0.5cm}
  \begin{minipage}[t]{0.49\linewidth}
    \vspace{0mm}
