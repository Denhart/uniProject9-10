...
# @param kwargs Arguments passed on to matplotlib.pyplot.plot.
def sparam(f, s, c="-", **kwargs):
    f *= freqscale(f)
    plt.plot(f, s, c, **kwargs)
...
def efficiency(f, e, c="-", **kwargs):
    ...
    lb_h = plt.plot(f_LB, np.interp(f_LB, f, e), c, **kwargs)
    if fmax >= 1710:
...
def correlation(f, ecc, c="-", **kwargs):
    f *= freqscale(f)
    ...
    lb_h = plt.plot(f_LB, np.interp(f_LB, f, ecc), c, **kwargs)
    if fmax >= 1710:
