...
f_L,eff_L = satimo.efficiency("satimo/antenna/wonder-top-noTuner-LB-H.trx", f_tot=f_tot, P_tot=P_tot)
f_H,eff_H = satimo.efficiency("satimo/antenna/wonder-top-noTuner-HB-H.trx", f_tot=f_tot, P_tot=P_tot)
f = hstack((satimo.ma(f_L,avg),satimo.ma(f_H,avg)))
eff = hstack((satimo.ma(eff_L,avg), satimo.ma(eff_H,avg)))
aauplot.efficiency(f,eff, '-b', label="Top")

# SIDE ANTENNA #################################################################
print("Processing side antenna...")

f_L,eff_L = satimo.efficiency("satimo/antenna/wonder-side-noTuner-LB-H.trx", f_tot=f_tot, P_tot=P_tot)
f_H,eff_H = satimo.efficiency("satimo/antenna/wonder-side-noTuner-HB-H.trx", f_tot=f_tot, P_tot=P_tot)
f = hstack((satimo.ma(f_L,avg),satimo.ma(f_H,avg)))
eff = hstack((satimo.ma(eff_L,avg), satimo.ma(eff_H,avg)))
aauplot.efficiency(f,eff, '-g', label="Side")
