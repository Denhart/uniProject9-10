...
The schematics are shown in Figure~\ref{fig:rffe_schematic_pc} and Figure~\ref{fig:rffe_schematic_ant}. Both the transmitter and receiver is included on the PC-side board. On the PC side, the RX and TX lines are connected to a PC using an USB to UART adapter (PC2102) with external connections for an FTDI breakout board. On the antenna side, the RFFE\_SCLK, RFFE\_SDATA, and Vrffe lines are connected to the WS1040 chip.
...
\subsection{Testing}
An antenna board with tuner has been measured on a VNA and in the Satimo chamber, with and without the RFFE adaptor board to check if the S-parameters or total efficiency changes with the adaptor board present. The S-parameters, shown in Figure~\ref{fig:rffe_test_sparams}, shows no significant change.

\begin{figure}[htbp]
    \centering
    \includegraphics{img/optical_rffe/sparam_comparison.pdf}
    \caption{S-parameter comparison of the WiSpry board with and without the battery and the RFFE adaptor PCB.}
    \label{fig:rffe_test_sparams}
\end{figure}
