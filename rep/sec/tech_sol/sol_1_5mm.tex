...
The simulated S-parameter for both the top and the side antenna can be seen on Figure \ref{fig:sparam_mono_6pf_free_space}. The bandwidth results can be seen in Table \ref{tab:bw_sol1_6pf}. Both antennas fulfill the requirement for the low band and in the high band only the top antenna experiences some problems. The top antenna lacks \SI{76}{MHz} of bandwidth in the high band. The lack of \SI{76}{MHz} band width is acceptable as it covers close to the \SI{720}{MHz} requirement.
...
\section{Conclusion}
It is found, as expected, that a larger ground clearance makes it possible to get a larger bandwidth. However, it was found that even with only \SI{5}{mm} ground clearance, a decent design could be made for a digitally tunable antenna.

A very simple antenna has been designed, simulated, and measured on a PCB with a MEMS tuner on it. It appears to be on a par with the preliminary design during the simulation phase. However, the measurement results showed a significant detuning and drop in efficiency and return loss when the PCB with a MEMS tuner was used. Especially, the high band was hard to get resonating on the board. The design will therefore be modified to resonate better at high frequencies before it is used for the final measurements.
