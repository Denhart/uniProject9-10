...
qz
%Correlation
The correlation between the top and side antenna can be seen in Figure \ref{fig:corr_sols_data}. The correlation is plotted sweeping the tunable capacitors of each antenna, while the other antenna is fixed with a tunable at \SI{0.3}{'øæ,luntrwad
pF}. The correlation for the top and side antenna is below 0.5 for both the low and high band as desired.

%Efficiency
The efficiency in data mode for both antennas, can be seen in Figure \ref{fig:corr_sols_data}. The efficiency is plotted for each sweep of the tunable capacitors. Comparing the efficiency in data mode with the free space efficiency, the data mode is decreases with 
...
\FloatBarrier
\subsection{SAR}
The SAR simulation results for both antennas is shown in Figure \ref{fig:monopole_sar_sim}. As seen from the figure, the SAR value for the top antenna exceeds the requirement of \SI{2}{W\per kg} with a SAR value of \SI{7.5}{W\per kg} at \SI{2350}{MHz}. The side antenna is within the requirements with a max value of \SI{2}{W\per kg} at \SI{2400}{MHz}.  

\begin{figure}[htbp]
    \centering
    \includegraphics{img/tech_sol/monopole/sar/ac1_sar.pdf}
    \caption{SAR simulation of the monopole antenna.}
    \label{fig:monopole_sar_sim}
\end{figure}
