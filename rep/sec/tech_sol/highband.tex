\section{Modified Minimized Antenna}
...
The isolation is rather low for the free-space simulation with a minimum isolation of \SI{6}{dB} in the low band. The isolation in the high band is generally high with a isolation above \SI{13}{dB}.
...
The lowest isolation simulated is at \SI{6}{dB} in the low band. Generally, the isolation is high in the high band but could cause some capacity problems in the low band. 
...
The isolation is slightly higher than the data mode, with a minimum isolation of \SI{7}{dB} in the low band. In the high band, the isolation is above \SI{14}{dB} and should not cause any problems.
...
As a results of the loss from both the head and the hand, the isolation is generally high for both antennas. The minimum isolation is within the low band at \SI{13}{dB} which is quite high compared to the data and play mode.  
...
The data and play mode experiences some problems with the isolation loss and some improvements might be necessary to increase the isolation.
...
The measured isolation is generally higher than the simulated. The measured low band isolation is \SI{6}{dB} higher than the simulated for both antennas. In the high band, it is the opposite case, where the simulation is higher than the measured, especially for the top antennas. However, this should not be a problem as the measured isolation is above \SI{12}{dB} in the required bandwidth.
