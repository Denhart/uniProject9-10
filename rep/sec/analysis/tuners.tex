...
The S parameters are easily measured using a Vector Network Analyzer (VNA). The advantage of measuring S-parameters over, e.g., Y-parameters is that all ports are matched during the measurement instead of being shorted, making odd behavior (like oscillations) less likely to occur \cite{}<++>.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        \begin{smithchart}
            \addplot coordinates {(1,0) (1,1) (1,2)};
        \end{smithchart}
    \end{tikzpicture}
    \caption{Smithchart.}
